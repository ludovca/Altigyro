...
def calcul_trajectoire():
    ...
    for i in range(len(t)):
        ...
        altitude = 44330 * (1 - (df['pressure'][i] / 1013.25) ** (1/5.257))
...
def analyse():
    ...
    a_tot = np.sqrt(df['accel_x']**2 + df['accel_y']**2 + df['accel_z']**2)
    a_max = np.max(a_tot)
    print(f"Accélération maximale : {a_max:.2f} m/s²")

    g_max = a_max / 9.81
    print(f"Force maximale en g : {g_max:.2f} g")

    v = np.zeros(len(a_tot) + 1)
    for i in range(len(a_tot)):
        if i < len(a_tot) - 1:
            dt = (df['time'][i + 1] - df['time'][i]) * 0.001
        v[i+1] = v[i] + a_tot[i] * dt
    v_max = np.max(v)
    print(f"Vitesse maximale : {v_max:.2f} m/s")
