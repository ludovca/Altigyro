...
*   **Génération du rapport à la volée :** Sans jamais interrompre ni fermer le flux d'enregistrement continu, le programme calcule les statistiques clés du vol ($H_{max}$, $V_{max}$, $G_{max}$, vitesse de rotation...) et vient insérer cette section de **conclusion automatique** au cœur des logs. L'acquisition des données brutes continue ensuite normalement en dessous.
